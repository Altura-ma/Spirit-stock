...
import { Bottle, BottleCategory, CATEGORY_LABELS } from '../types';

const CATEGORIES = Object.entries(CATEGORY_LABELS) as [BottleCategory, string][];

export default function EditBottleScreen({ navigation, route }: any) {
  const { bottle }: { bottle: Bottle } = route.params;
  const { updateBottle } = useStock();
  const { suppliers } = useStock();

  const [name, setName] = useState(bottle.name);
  const [category, setCategory] = useState<BottleCategory>(bottle.category);
  const [quantity, setQuantity] = useState(String(bottle.quantity));
  const [minThreshold, setMinThreshold] = useState(String(bottle.minThreshold));
  const [price, setPrice] = useState(String(bottle.price));
  const [supplierId, setSupplierId] = useState(bottle.supplierId);
  ...
  const handleSave = async () => {
    ...
    if (isNaN(qty) || qty < 0 || isNaN(min) || min < 0 || isNaN(p) || p < 0) {
    ...
    <SafeAreaView style={styles.safe}>
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={styles.flex}>
        <View style={styles.navBar}>
          ...
          <Text style={styles.navTitle}>Modifier le produit</Text>
              ...
              <>
                ...
                >
                  <Text style={styles.pickerText}>
                    {selectedSupplier?.name || 'Sélectionner'}
                  </Text>
